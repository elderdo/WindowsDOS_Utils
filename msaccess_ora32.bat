...
setlocal
set MSACCESS="C:\Program Files (x86)\Microsoft Office\Office15\MSACCESS.exe"
if not EXIST %MSACESS% goto noMsAccess
for %%i in (%MSACCESS%) do set MSACCESS_SHORT_NAME=%%~si

if not C:\Oracle\NUL goto noOracle

:: find the lastest 32 bit directory for Oracle
for /f "tokens=*" %%a in ('dir /b /od C:\Oracle\*32*') do set newest=%%a
