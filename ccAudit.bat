...
if "%ename%"=="ksh" set VOB_DIR=properties
if "%ename%"=="ctl" set UNIX_DIR=src
if "%ename%"=="ksh" set VOB_DIR=ctl
if "%ename%"=="sql" set UNIX_DIR=src
if "%ename%"=="ksh" set VOB_DIR=sql
...
if NOT %ERRORLEVEL%==0 (
	...
	if not exist %1 (
		@echo unable to download %1 via ftp 
		@echo must correct %1 manually
		goto endAudit
	)
