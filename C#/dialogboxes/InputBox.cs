...
namespace RobvanderWoude
{
	class InputBox
	{
		public const string progver = "1.33.1";
		...
		static int Main( string[] args )
		{
			...
			foreach ( string arg in args )
			{
				if ( arg[0] == '/' )
				{
					...
					else if ( arg.Length == 2 )
					{
						switch ( arg.ToString( ).ToUpper( ) )
						{
							case "/A":
								if ( ASCIIonly )
								{
									return ErrorMessage( "Duplicate command line switch /A" );
								}
								ASCIIonly = true;
								break;
							case "/B":
								if ( bw )
								{
									return ErrorMessage( "Duplicate command line switch /B" );
								}
								bw = true;
							...
							case "/L":
								...
								{
									return ErrorMessage( "Duplicate command line switch /L" );
								}
							...
							case "/N":
								if ( !filtered )
								{
									return ErrorMessage( "Duplicate command line switch /N" );
								}
								filtered = false;
								break;
							case "/P":
								if ( password )
								{
									return ErrorMessage( "Duplicate command line switch /P" );
								}
								password = true;
								break;
							case "/S":
								if ( showpassword )
								{
									return ErrorMessage( "Duplicate command line switch /S" );
								}
								showpassword = true;
								break;
							case "/T":
								if ( timeoutset )
								{
									return ErrorMessage( "Duplicate command line switch /T" );
								}
								timeout = deftimeout;
								timeoutset = true;
								break;
							default:
								return ErrorMessage( "Invalid command line switch {0}", arg );
						}
					}
					else if ( arg.Length == 3 )
					{
						...
						{
							case "/M:":
								ErrorMessage( "No mask specified with /M" );
								Console.WriteLine( "\n\n" );
								return HelpMessage( "mask" );
							default:
					...
					else
					{
						switch ( arg.Substring( 0, 3 ).ToUpper( ) )
						{
							case "/H:":
								if ( heightset )
								{
									return ErrorMessage( "Duplicate command line switch /H" );
								}
								try
								{
							...
							case "/M:":
								if ( usemask )
								{
									return ErrorMessage( "Duplicate command line switch /M" );
								}
							...
							case "/R:":
								if ( regexset )
								{
									return ErrorMessage( "Duplicate command line switch /R" );
								}
							...
							case "/W:":
								if ( widthset )
								{
									return ErrorMessage( "Duplicate command line switch /W" );
								}
								try
								{
			...
			{
				return ErrorMessage( "Command line switch /A (ASCII only) can only be used together with /M" );
			}
			...
			if ( localizedcaptionset )
			{
				...
				if ( !String.IsNullOrWhiteSpace( localizationstring ) )
				{
							...
							if ( !String.IsNullOrWhiteSpace( val ) )
							{
								switch ( key.ToUpper( ) )
								{
									...
									case "CANCEL":
										...
										break;
									default:
										return ErrorMessage( "Invalid localization key \"{0}\"", key );
								}
							}
						}
					}
					...
					{
						return ErrorMessage( "Invalid localization string:\n\t{0}", localizationstring );
					}
				}
			}
		...
		public static int ErrorMessage( params string[] errmsg )
		{
			...
			*/
			...
			Console.ForegroundColor = bold;
			Console.Error.WriteLine( "InputBox.exe \"Enter a MAC address:\" \"MAC Address\" \"0022446688AA\"" );

			Console.Error.WriteLine( "             /M:\">CC\\:CC\\:CC\\:CC\\:CC\\:CC\" /R:\"[0-9A-F]{2}(\\:[0-9A-F]{2}){5}\"" );
			Console.ResetColor( );
		...
		public static int ValidateAndShowResult( )
		{
			...
			if ( regexset && !new Regex( pattern, RegexOptions.IgnoreCase ).IsMatch( input ) )
			{
				return 4;
			}
			...
			if ( filtered )
			{
				input = new Regex( @"[&<>|]" ).Replace( input, String.Empty );
			}
