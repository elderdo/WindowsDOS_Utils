...
namespace RobvanderWoude
{
	class DropDownBox
	{
		public static string progver = "1.11.1";
		public static bool returnindex = false;
		...
		static int Main( string[] args )
		{
			try
			{
					...
					else
					{
						switch ( arg.Substring( 0, 3 ).ToUpper( ) )
						{
							...
							case "/RI":
								if ( returnindex )
								{
									return ShowHelp( "Duplicate command line switch /RI" );
								}
								returnindex = true;
								break;
							case "/T:":
				...
				foreach ( string item in list.Split( delimiter ) )
				{
				...
				if ( result == DialogResult.OK )
				{
					...
					return ( returnindex ? combobox.SelectedIndex + 1 : 0 );
				}
				else
				{
					return ( returnindex ? 0 : 2 );
				}
			}
			catch ( Exception e )
			{
		...
		public static int ShowHelp( params string[] errmsg )
		{
			...
			*/
			...
			Console.ForegroundColor = ConsoleColor.White;
			Console.Error.Write( "Options:  /W:width" );
			Console.ResetColor( );
			...
			Console.Error.Write( "/F:\"listfile\"" );
			Console.ResetColor( );
			Console.Error.WriteLine( ", not both." );
